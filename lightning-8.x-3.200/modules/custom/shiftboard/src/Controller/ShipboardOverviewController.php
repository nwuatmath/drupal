...
namespace Drupal\shipboard\Controller;

use Drupal\Core\Controller\ControllerBase;
use Drupal\shipboard\ShipboardConnection;

/**
 * Provides controller methods for the Shipboard API integration overview.
 */
class ShipboardOverviewController extends ControllerBase {
    ...
    /**
     * Ping the Shipboard API for basic data.
     *
    ...
    protected function pingEndpoint(&$build) {
        ...
        $json       = NULL;
        try {
            $response = $connection->callEndpoint('accountsDetailFull', [
                'limit'     => 10,
                'url_query' => [
                    'sort' => 'gid asc',
                ]
    ...
    protected function displayDataSample($json, &$build) {
        if (isset($json->response_data[0])) {
            $account_data = $json->response_data[0];
            $build['account_sample'] = [
                '#prefix' => '<pre>',
                '#markup' => print_r($account_data, TRUE),
                '#suffix' => '</pre>',
            ];
        }
    }
